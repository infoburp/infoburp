...
infoburp.command = function(graph) {
...
infoburp.force.tick = function(graph) {
...
infoburp.graph = function(config) {
    ...
    this.socket      = new io.connect(infoburp.conf.core.host);
    this.command     = new infoburp.command(this);
    this.ui          = new infoburp.ui(this);
    this.handler     = new infoburp.eventHandler(this);

    this.settings    = infoburp.conf.graphSettings;
    this.debug       = false;
    this.refs.div    = this.ui.identifiers.graphPlaceholder;
    this.updateMode  = 'replace';           // {'update'|'replace'} for new svg data

    this.width       = infoburp.conf.graphSettings.width;
    this.height      = infoburp.conf.graphSettings.height;
...
infoburp.graph.prototype.refresh  = function() {
...
infoburp.graph.prototype.addGraphData = function(graphData) {
...
infoburp.graph.prototype.addNode = function(node) {

    if(this.nodeCount() > infoburp.conf.graphSettings.nodeLimit) {
        console.log("WARNING: Node count exceeds max (" +
            infoburp.conf.graphSettings.nodeLimit + ")" +
            "set by infoburp.conf.graphSettings - ignoring");
        return;
    }
...
infoburp.graph.prototype.resetLinks = function() {
    var graph = this;
...
infoburp.graph.prototype.removeNode  = function(id) {
...
infoburp.graph.prototype.addLink = function(sourceIndex, targetIndex, relType) {
    ...
    if(this.data.nodes[sourceIndex] !== undefined && this.data.nodes[targetIndex] !== undefined) {
        ...
        var link = new infoburp.Link(source, target, relType);
    }
...
infoburp.graph.prototype.removeLink  = function(source, target, rel) {
...
infoburp.graph.prototype.linkExists  = function(source, target, rel) {
...
infoburp.graph.prototype.checkLinks = function() {
...
infoburp.graph.prototype.addNodeType = function(nodeType) {
...
infoburp.graph.prototype.addLinkType = function(linkType) {
...
infoburp.graph.prototype.selectNode = function(node, element) {
    this.state.selectedNode={ data: node, elem: element };
};

infoburp.graph.prototype.unselectNode = function() {
 ...
 *
 *  {infoburp.Node} source origin node for this connection
 *  {infoburp.Node} target destination node for this connection
 *  {string} rel name of this relationship type
**/

infoburp.graph.prototype.connectNodes = function(source, target, rel) {
...
infoburp.Link = function(source, target, rel) {
    this.source = source;
    this.target = target;
    this.rel = rel;
};
/**
 *  infoburp Node object
 *
 *  @constructor
 *  @this {node}
...
infoburp.Node = function(graffnode) {
    ...
    this.getIcon = function() {
        if(this.type === undefined)
            return "?";
        if(infoburp.conf && infoburp.conf.nodeIconFor) {

            if(infoburp.conf.nodeIconFor[this.type])
                return infoburp.conf.nodeIconFor[this.type];

            if(infoburp.conf.nodeIconFor['default'])
                return infoburp.conf.nodeIconFor['default'];
...
infoburp.util.objectToForm = function(data, opts) {
    var str = "";
    Object.keys(data).forEach(function(key){
       // str += '<div>';
//        str += '<label class="form-label">' + key + '</label>';
        if(opts[key] === undefined)
          str += infoburp.util.jsToFormField(data[key], key, "", 14);
        else
          str += infoburp.util.jsToFormField(opts[key].data, key, null, null, opts[key].selected, opts[key].user);
        //str += '</div>';
    });
    return str;
};

infoburp.util.relsArrayToHTML = function(nodeId, rels) {
...
infoburp.util.relsTypesArrayToHTML = function(rels) {
...
infoburp.util.paramDefault = function(param, def) {
...
infoburp.util.jsToFormField = function(jsObj, name, id, size, sel, user) {

  var type  = infoburp.util.getType(jsObj);
  var id    = infoburp.util.paramDefault(id,      '');
  var size  = infoburp.util.paramDefault(size,    50);
  var name  = infoburp.util.paramDefault(name,    '');
  var sel   = infoburp.util.paramDefault(sel,   null);
  var user  = infoburp.util.paramDefault(user, false);
  ...
  switch(type) {
    ...
    case "number" :
      return '<input type="text" jsontype="' + type + '" name="' + name + '" id="textarea" size="' + size + '" value="' + jsObj +'" />';
      break;
...
infoburp.util.getType = function(obj) {
  var s = typeof obj;
...
infoburp.util.getFormValues = function(rootNode) {
    var result = [];
    var currentNode = rootNode.firstChild;

    while (currentNode) {
          if (currentNode.nodeName.match(/INPUT|SELECT|TEXTAREA/i)) {
                result.push({ name: currentNode.name, value: infoburp.util.getFieldValue(currentNode) });
          }
          else {
                var subresult = infoburp.util.getFormValues(currentNode);
...
infoburp.util.getFieldValue = function(fieldNode) {
    if (fieldNode.nodeName == 'INPUT') {
          ...
          else
          {
                if (!fieldNode.type.toLowerCase().match(/button|reset|submit|image/i)) {
                      switch(infoburp.util.getFieldType(fieldNode)) {
                            case 'string':
          ...
          if (fieldNode.nodeName == 'TEXTAREA') {
                ...
                if (fieldNode.nodeName == 'SELECT') {
                      return infoburp.util.getSelectedOptionValue (fieldNode);
                }
          }
    }
...
infoburp.util.getFieldType = function(fieldNode) {
  return fieldNode.attributes.getNamedItem('jsontype').value;
};

infoburp.util.getSelectedOptionValue = function(selectNode) {
...
infoburp.graph.prototype.makeLabelsRoot = function() {
...
infoburp.graph.prototype.drawLabels = function() {
...
infoburp.graph.prototype.makeNodeRoot = function() {
...
infoburp.graph.prototype.drawNodes = function() {
    ...
    this.refs.circle.enter()
        .append("svg:circle")
            ...
            .attr("r", infoburp.conf.graphSettings.circleRadius)
...
infoburp.graph.prototype.makePathIconRoot = function() {
...
infoburp.graph.prototype.drawPathIcons = function() {
...
infoburp.graph.prototype.makePathRoot = function() {
...
infoburp.graph.prototype.drawPaths = function() {
...
infoburp.graph.prototype.makeSvg = function() {
    ...
    this.refs.force = d3.layout.force()
        ...
        .on("tick", infoburp.force.tick(this));
...
function zoom() {
  graph.attr("transform", "translate(" + d3.event.translate + ")scale(" + d3.event.scale + ")");
    ...
    infoburp.graph.linkMenu = d3.select("body").append("div")
        .attr("class", "linkMenu")
        .style("opacity", 0);

    // mouse-out event for links/relations menu

    infoburp.graph.linkMenu
        .on("mouseout", function(d) {
            infoburp.graph.linkMenu.transition()
                .duration(500)
                .style("opacity", 0);
        });
    ...
    infoburp.graph.defs = this.refs.svg.append('svg:defs');
    ...
    **/
    ...
    var filter = infoburp.graph.defs
        .append("svg:filter")
            .attr("id", "dropshadow")
            .attr("height", "180%");
...
infoburp.ui.prototype.dialogs = function() {
...
infoburp.ui.prototype.init = function() {
...
infoburp.ui.prototype.updateNodeTypes = function() {
    var ui = this;
...
infoburp.ui.prototype.addUniqueSelectOption = function(select, value, text) {
...
infoburp.ui.prototype.updateStats = function() {
...
infoburp.ui.prototype.updateNeoData = function(stat, value) {
...
infoburp.ui.prototype.forceStart = function() {
...
infoburp.ui.prototype.forceStop = function() {
...
infoburp.ui.prototype.showNodeEditForm = function(node) {
    graph.debugMesg("(showNodeEditForm) showing form for node " + node.id);
    $(this.identifiers.nodeEditableData).html(infoburp.util.objectToForm(node.data, { type: { data: graph.data.nodeTypes, user: true, selected: node.type }}));
...
console.log (infoburp.util.objectToForm(node.data, { type: { data: graph.data.nodeTypes, user: true, selected: node.type }}));
...
infoburp.ui.prototype.updateWithConsoleNodeId = function(node) {
...
infoburp.ui.prototype.clearNodeMenuData = function() {
    $('#graph-start').val(0);
    $(this.identifiers.nodeEditableData).html("");
    this.disableActionButtons(true);
};
...
infoburp.ui.prototype.showNodeInformation = function(node) {
    ...
    var rels = infoburp.util.relsArrayToHTML(node.id, node.getRels(graph));
    $(this.identifiers.nodeInfoRels).html(rels);
    var left  = node.x + (infoburp.conf.graphSettings.circleRadius * 2) - 5;
    var right = node.y + 40;
...
infoburp.ui.prototype.showNodeMenu = function(node) {
...
infoburp.ui.prototype.disableActionButtons = function(active) {
...
infoburp.ui.prototype.disableGraphActionButtons = function(active) {
    $(this.buttons.graphForceStart).prop('disabled', active);
    $(this.buttons.graphForceStop).prop('disabled', active);
};

infoburp.ui.prototype.markNodeAsSelected = function(element) {
    element.setAttribute("class",
        element.getAttribute("class") + " " + infoburp.conf.graphSettings.selectedClass);
};

infoburp.ui.prototype.markNodeAsUnselected = function(element) {
    var re = new RegExp(" " + infoburp.conf.graphSettings.selectedClass, 'g');
...
infoburp.ui.prototype.showNewRelationshipMenu = function(nodeSource, nodeTarget) {
...
infoburp.ui.prototype.clearDrag = function() {
...
infoburp.ui.prototype.registerButtonClicks = function() {
    ...
    $("#node-edit-update").click(function(e) {
        var newObj = infoburp.util.formToObject('node-data');
    ...
    $(this.buttons.nodeDuplicate).click(function(e) {
        ...
        var newObj = infoburp.util.formToObject(graph.ui.identifiers.nodeEditableData.replace(/#/,''));
...
infoburp.ui.prototype.identifiers = {
...
/**
 *  infoburp UI event handlers
 *
 *  @namespace infoburp.eventHandler
 */

infoburp.eventHandler = function(graph) {
...
infoburp.eventHandler.prototype.dragletDrag = function() {
    return function(d,i){
        ...
        var dragletx = parseInt(d3.mouse(document.getElementById('graph'))[0], 0);
...
infoburp.eventHandler.prototype.dragletDragEnd = function() {
    return function(d,i){
...
infoburp.eventHandler.prototype.linkMouseover = function() {
    return function(d, i) {
...
infoburp.eventHandler.prototype.linkMouseout = function() {
    return function(d, i) {
...
infoburp.eventHandler.prototype.linkRightClick = function() {
    return function(d, i) {
...
infoburp.eventHandler.prototype.nodeClick = function() {
    return function(d, i) {
...
infoburp.eventHandler.prototype.nodeDoubleClick = function() {
    return function(d, i) {
...
infoburp.eventHandler.prototype.nodeMouseover = function() {
    return function(d, i) {
...
infoburp.eventHandler.prototype.nodeMouseout = function() {
    return function(d, i) {
...
infoburp.eventHandler.prototype.newRelSelected = function(newRelType) {
    if(newRelType !== null) {
...
infoburp.eventHandler.prototype.deleteRelButtonClick = function(source, target, rel) {
