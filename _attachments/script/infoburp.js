...
dragged_node_number=null; //makes it so no node is being dragged.
dragged_link_number=null; //makes it so no link is being dragged.
// add some code to size the layout according to the viewport size.
var width = 960, 
    height = 500, 
    fill = d3.scale.category20();
// persistence/basic_persistence.js 
//show edits on the graph and store it all in couchdb.
var previous_graph_state = restore_graph_state();
//get the data ready to feed in
global_data={nodes:[], links:[]};
global_data.nodes = previous_graph_state.nodes;
global_data.links = restore_links(previous_graph_state);
//define the infoburp layout
...
function dragstart(d, i) {
	...
	dragged_link_number=global_data.links.push({source:d,target:new_node})-1;
        if (verbose = true){alert("Added node");};
	restart();
    }
}
...
if ( d3.event.sourceEvent.srcElement.className!=="blockdragging"){
	...
	global_data.nodes.forEach(function(target,num){
					  ...
					  if (verbose = true){alert("Added link");};
					  flag=false;
					  restart();
				      };
				}
