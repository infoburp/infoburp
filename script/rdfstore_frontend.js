(function(){
    ...
    RDFStoreFrontend.prototype.buildTemplates = function(node) {
        ...
        html = html + "<tr class='{{if $index%2==0}}sparql-result-even-row{{else}}sparql-result-odd-row{{/if}}'>{{each $value }}{{if $value==null}}";
        html = html + "<td><span class='rdfstore-data-value'>&nbsp;</span></td>{{else $value.token==='uri'}}";
